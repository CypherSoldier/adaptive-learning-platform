...
def test_user_profile(client, auth_headers):
    response = client.get("/users/profile?user_id=1&track_id=1", headers=auth_headers)
    assert response.status_code == 200
    profile_data = response.json()
    assert "confidence_score" in profile_data
    assert "skill_score" in profile_data


def test_user_all_profiles(client, auth_headers):
    response = client.get("/users/skill-profiles", headers=auth_headers)
    assert response.status_code == 200
